...
module.exports = {
  ...
  artifactName: `dsh-nexus_${version}_${platform}_\${arch}_portable.\${ext}`,
  asar: true,
  ...
  nsis: { artifactName: `dsh-nexus_${version}_${platform}_\${arch}.exe`, oneClick: false, perMachine: false, allowToChangeInstallationDirectory: true,
    differentialPackage: false, installerLanguages: ['en_US', 'zh_CN', 'zh_TW', 'ja_JP', 'ko_KR', 'de_DE', 'fr_FR', 'es_ES'],
    deleteAppDataOnUninstall: false },
  dmg: { artifactName: `dsh-nexus_${version}_${platform}_\${arch}.dmg` },
  linux: { artifactName: `dsh-nexus_${version}_${platform}_\${arch}.\${ext}`, target: ['AppImage', 'deb', 'rpm'], executableName: 'nexus-launcher', icon: 'desktop/icons',
  ...
  deb: { depends: ['libc6 (>= 2.34)', 'libcurl4', 'zlib1g', 'libgtk-3-0', 'libnss3', 'libasound2', 'libgbm1', 'libxss1', 'libatk-bridge2.0-0', 'libdrm2', 'libxkbcommon0'] },
  rpm: { depends: ['glibc >= 2.34', 'libcurl', 'zlib', 'gtk3', 'nss', 'alsa-lib', 'mesa-libgbm', 'libXScrnSaver', 'at-spi2-atk', 'libdrm', 'libxkbcommon'] },
