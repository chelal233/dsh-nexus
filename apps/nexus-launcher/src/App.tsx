    ...
    <>
      ...
      <div className="grid-two">
        <Panel title={t("Agent operations")} icon={<Pulse size={18} />}>
          <p className="panel-description">{t("The Agent starts and stops with the Launcher. Only force-restart remains for recovery.")}</p>
          <div className="button-row">
            <ActionButton tone="primary" disabled={agentRestartDisabled} onClick={() => void runAction(t("Force restart Agent"), "/v1/agent", { action: "restart" })}><ArrowsClockwise size={16} />{t("Force restart Agent")}</ActionButton>
          </div>
        </Panel>
...
export function ProfilesView(props: ViewProps) {
  ...
  const viewing = viewProfile || active;
  ...
  return <><PageIntro kicker={t("Control / Profiles")} title={t("Profiles")} detail={t("Profiles own checkpoints and the plugin inventory: select a profile, manage its checkpoints, then adjust its plugins. Profile creation and deletion are unavailable in this release.")} /><Panel title={t("Profile catalog")} icon={<SlidersHorizontal size={18} />}>
    ...
    <DataList items={manifests} emptyTitle={t("No valid native profiles")} emptyDetail={t("Only valid profile manifests are selectable.")} render={(item) => { const name = stringValue(item, "name") || t("Unnamed profile"); const isViewing = name === viewing; return <><div><strong>{name}</strong>{name === active && <StatusPill label={t("Active")} tone="good" />}{isViewing && <StatusPill label={t("Viewing")} tone="neutral" />}<span>{t("{count} bundles", { count: arrayValue(item, "bundles").length })}</span></div><span className="row-meta">{!isViewing && <ActionButton onClick={() => setViewProfile(name)}>{t("View")}</ActionButton>}{name !== active && <ActionButton disabled={gate.disabled} onClick={() => void runAction(t("Profile selection"), "/v1/profiles", { action: "select", profile: name })}>{t("Select")}</ActionButton>}</span></>; }} />
  </Panel>
  {viewing && <div className="profile-children"><div className="profile-children-title">{t("Belongs to profile")}: {viewing}</div>
  <CheckpointsView {...props} embedded profileFilter={viewing} />
  <ProfilePlugins {...props} profile={viewing} />
