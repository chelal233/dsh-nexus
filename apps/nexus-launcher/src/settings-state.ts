...
export function offlineImportDefaults(data: Record<string, unknown> | null | undefined) {
  return { runtime: data?.runtime !== false, profiles: Array.isArray(data?.profiles) ? data.profiles.map(String) : [],
    configuration: data?.configuration === true, environment: (data?.environment ?? data?.configuration) === true,
    sessions: data?.sessions === true, plugins: data?.plugins === true,
    credentials: false, credential_policy: "preserve" as "preserve" | "replace" };
}

export function releasePromotionCommand(id: string, confirmation: string | null, accepted: boolean): Record<string, unknown> | null {
  if (confirmation && !accepted) return null;
  return { action: "promote", id, ...(confirmation ? { rollback_confirmation: confirmation } : {}) };
}
...
export function cleanupGroups(areas: Record<string, unknown>[], items: Record<string, unknown>[]) {
  const normalize = (value: unknown) => String(value ?? "").replace(/\\/g, "/").replace(/\/+$/, "").toLowerCase();
  const groups = areas.map(area => ({ area, items: [] as Record<string, unknown>[] }));
  const other: Record<string, unknown>[] = [];
  for (const item of items) {
    const path = normalize(item.path);
    const candidates = groups.filter(group => { const root = normalize(group.area.path); return root && (path === root || path.startsWith(root + "/")); });
    candidates.sort((a,b) => normalize(b.area.path).length - normalize(a.area.path).length);
    if (candidates.length) candidates[0].items.push(item); else other.push(item);
  }
  if (other.length) groups.push({ area: { kind: "Other cleanup items", path: "" }, items: other });
  return groups;
}

export function toggleCleanupGroup(selected: string[], items: Record<string, unknown>[], checked: boolean): string[] {
  const eligible = items.filter(item => item.eligible === true && typeof item.id === "string").map(item => String(item.id));
  return checked ? [...new Set([...selected, ...eligible])] : selected.filter(id => !eligible.includes(id));
}
