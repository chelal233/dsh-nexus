...
提交前在仓库根目录运行 `node .github/scripts/check-source-privacy.mjs`。文档用路径占位符，测试用通用用户名，不提交内部记忆、真实凭据或个人机器路径。此检查覆盖已跟踪文件的路径和敏感文件名，不替代完整凭据扫描；构建与发布流程均强制执行。
...
根目录首页例外：`README.md` 默认为英文，`README.zh-CN.md` 为中文；`README.en.md` 仅保留旧链接入口。

发布时同步中英文变更记录与发布正文，按用户可感知的变化、使用方式和限制组织，遵循[发布说明标准](docs/github-release.md)。增加运行依赖或平台时，同时更新支持矩阵、第三方材料和对应验收范围；不能仅修改 README。
