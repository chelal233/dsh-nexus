...
### 已落地（全部 styles.css + 一处按钮 tone）
- **块级语言三态**：Metric 状态卡=实底浮卡（surface+圆角10）；空状态/错误卡=凹陷槽（黑 α0.22 + 浅色主题覆盖）；Panel=渐变+阴影（原有）——「浮卡/槽位/面板」三级一眼可辨
- **引导步骤卡**：三步骤条升级为卡片体（边框+左 3px 导轨+底色），**step-done 完成态=绿导轨+绿字**（此前 JSX 已输出类但 CSS 无定义，状态语义完全不可见）
- **间距三档节奏**：组内 12 / 块间 16 / 章节标题 28（原 10/13/16/17/19/24/27px 七种混用归一）
- **字阶**：Metric 标签10px-subtle→值20px→详情11px-muted 三级；dt/dd 拉开一档且 dd 允许换行（长错误不再被 ellipsis 静默裁剪）；kicker 9→10px；输入控件统一 12px（与按钮同高）
- **一致性**：nav 选中态加 2px 内嵌导轨+加粗；顶栏工具条按钮压缩至 30px（让位页面主操作）；status-block 内按钮不再被 grid 拉伸全宽；原生 select 归一；kv-row 行样式补齐（此前无规则竖向堆叠）；折叠三角染强调色+hover 反馈；error-state 定宽/偏移收敛到 workspace 直下（面板内错误卡不再被强加页面级布局）；重启按钮 tone 跨页统一 primary
- **修复失效样式**：.diagnostic-bundle 引用不存在的 --border 变量（分隔线画不出）、panel-divider/profile-children 硬编码灰 → 全部归位 --line/--line-strong token

### 留档（增强项，非缺陷）
- `.metric strong` 将 warn 态「未连接」也染强调绿——语义建议交 StatusPill 承载（需 JSX 状态分类，改动面大，未动）
- 同一动作的 i18n/文案跨页措辞差异（如「拉取上游标签」vs「列出标签」）随文案统一批次处理

### 验证
- 五页逐页浏览器截图验收（层级/块感/一致性全部生效）；tsc+vite build 过；37/37 tests
