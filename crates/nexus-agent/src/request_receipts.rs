...
impl Receipts {
    ...
    fn load(&self, active: &HashSet<String>) -> io::Result<Document> {
        ...
        doc.requests.retain(|item| {
            active.contains(&item.request_id)
                || timestamp(&item.request_id).is_some_and(|issued| {
                    issued <= now.saturating_add(60) && now.saturating_sub(issued) <= LIFETIME
                })
        });
        if doc.requests.len() > LIMIT {
            return Err(io::Error::other("Request receipt capacity reached"));
        }
    ...
    fn read_document(&self) -> io::Result<Document> {
        let doc = match nexus_core::read_regular_file_bounded(
            &self.root.join("request-receipts.json"),
            1024 * 1024,
        )? {
        ...
        if doc.format_version != 1 {
            return Err(io::Error::other("Unsupported request receipt format"));
        }
        Ok(doc)
    }
    fn save(&self, doc: &Document) -> io::Result<()> {
        nexus_core::write_private_json_atomic(
            &self.root,
            &self.root.join("request-receipts.json"),
            doc,
        )
...
pub(crate) fn observed_context(paths: &nexus_core::NexusPaths) -> nexus_core::OperationContext {
    let profile = nexus_core::read_regular_file_bounded(&paths.profiles_file, 1024 * 1024)
        .ok()
        .flatten()
    ...
    nexus_core::OperationContext {
        ...
        profile,
        config_revision: nexus_core::ConfigStore::new(paths.clone())
            .snapshot()
            .ok()
            .map(|s| s.revision),
        run_id,
...
pub(crate) fn launch_context(paths: &nexus_core::NexusPaths) -> Option<Value> {
    let bytes = nexus_core::read_regular_file_bounded(
        &paths.run_dir.join("harness-log-session.json"),
        64 * 1024,
    )
    .ok()??;
    let session: nexus_core::HarnessLogSession = serde_json::from_slice(&bytes).ok()?;
    Some(
        json!({"run_id":session.run_id,"generation":session.generation,"created_at_unix":session.created_at_unix,"context":session.context}),
    )
}
pub(crate) fn diagnostic_summary(paths: &nexus_core::NexusPaths) -> Value {
    match Receipts::new(paths.root.clone()).read_document() {
        Ok(doc) => {
            json!({"state_source":"last_persisted; use /v1/requests for current owner status", "requests":doc.requests.iter().map(public).collect::<Vec<_>>()})
        }
...
fn reply(item: &Receipt) -> Response {
    ...
    if !matches!(item.state.as_str(), "running" | "completed") {
        ...
        body["code"] = json!(if item.state == "interrupted" {
            "request_interrupted"
        } else {
            "request_previous_failed"
        });
...
fn error(status: StatusCode, code: &str, message: &str) -> Response {
    super::api_error_response(status, code, message)
}
fn timestamp(id: &str) -> Option<u64> {
    let (time, random) = id.split_once('-')?;
    if !matches!(random.len(), 32 | 64)
        || !random
            .bytes()
...
pub(crate) async fn list(State(state): State<Receipts>) -> Response {
    ...
    match state.load(&active) {
        Ok(doc) => Json(json!({"requests":doc.requests.iter().map(public).collect::<Vec<_>>()}))
            .into_response(),
        Err(e) => error(
...
pub(crate) async fn enforce(
    ...
    if request.method() != axum::http::Method::POST
        || !matches!(
            request.uri().path(),
            "/v1/releases" | "/v1/updates" | "/v1/checkpoints" | "/v1/harness" | "/v1/profiles"
        )
    {
    ...
    let bytes = match to_bytes(body, BODY_LIMIT).await {
        ...
        Err(_) => {
            return error(
                ...
                "request_too_large",
                "Request exceeds the operation budget",
            )
        }
    };
    ...
    let Some(kind) = kind(parts.uri.path(), &value) else {
        return next
            .run(Request::from_parts(parts, Body::from(bytes)))
            .await;
    };
    let id = value
        ...
        .unwrap_or("")
        .to_owned();
    let Some(issued) = timestamp(&id) else {
        return error(
            StatusCode::BAD_REQUEST,
            "request_id_required",
            "Supply a stable timestamp-random request_id and reuse it after a timeout",
        );
    };
    ...
    let owner = tokio::spawn(async move {
        ...
        let response = response.unwrap_or_else(|_| {
            error(
                ...
                "request_interrupted",
                "Operation owner stopped unexpectedly; inspect recovery status",
            )
        });
        ...
        if let Err(e) = result {
            return error(
                StatusCode::SERVICE_UNAVAILABLE,
                "request_receipt_finalize_failed",
                &format!("Inspect recovery before retrying with the same request ID: {e}"),
            );
        }
        match bytes {
            ...
            Err(_) => error(
                ...
                "request_interrupted",
                "Operation response exceeded its budget; inspect recovery status",
            ),
        }
    });
    owner.await.unwrap_or_else(|_| {
        error(
            StatusCode::INTERNAL_SERVER_ERROR,
            "request_interrupted",
            "Inspect the original request and recovery status",
        )
    })
}
...
mod tests {
    ...
    fn malformed_and_expired_receipts_are_pruned_before_capacity_but_active_ids_survive() {
        ...
        let mut requests = (0..LIMIT + 1)
            .map(|i| make(format!("broken-{i}")))
            .collect::<Vec<_>>();
        requests.push(make(format!("{}-{:032x}", now - LIFETIME - 1, 1)));
        requests.push(make(fresh.clone()));
        requests.push(make(active_id.clone()));
        state
            .save(&Document {
                format_version: 1,
                requests,
        ...
        assert_eq!(doc.requests[0].request_id, fresh);
        assert_eq!(doc.requests[0].state, "interrupted");
        assert_eq!(doc.requests[1].request_id, active_id);
        assert_eq!(doc.requests[1].state, "running");
    ...
    async fn send(address: std::net::SocketAddr, path: &str, body: Value) -> (u16, Value) {
        ...
        let status = String::from_utf8_lossy(&bytes[..boundary])
            .split_whitespace()
            .nth(1)
            .unwrap()
            .parse()
    ...
    async fn receipt_retries_do_not_repeat_effects_and_restart_is_honest() {
        ...
        let handler = move || {
            ...
            async move {
                ...
                (
                    StatusCode::ACCEPTED,
                    Json(json!({"operation":{"operation_id":"cold-original"}})),
                )
            }
        };
        let app = Router::new()
            .route("/v1/releases", post(handler.clone()))
            .route("/v1/harness", post(handler.clone()))
            .route("/v1/updates", post(handler.clone()))
            .route("/v1/checkpoints", post(handler.clone()))
            .route("/v1/profiles", post(handler))
        ...
        let now = nexus_core::unix_time_seconds();
        for (index, path, action) in [
            (0, "/v1/releases", "rollback"),
            (1, "/v1/updates", "switch"),
            (2, "/v1/updates", "offline_import"),
            (3, "/v1/checkpoints", "restore"),
            (4, "/v1/harness", "restart"),
            (5, "/v1/profiles", "delete"),
            (6, "/v1/profiles", "restore_deleted"),
        ] {
            let id = format!("{now}-{index:032x}");
            let body = json!({"request_id":id,"action":action,"id":"safe-target"});
            let (first, second) = tokio::join!(
                send(address, path, body.clone()),
                send(address, path, body.clone())
            );
        ...
        assert_eq!(
            send(
                address,
                "/v1/releases",
                json!({"action":"rollback","request_id":format!("{}-{:032x}",now-LIFETIME-1,99)})
            )
            .await
            .0,
            409
    ...
    async fn receipt_failure_preserves_original_error_and_timeout_status() {
        ...
        let app = Router::new()
            .route(
                "/v1/releases",
                post(|| async {
                    super::super::data_error_response(
                        io::Error::new(io::ErrorKind::TimedOut, "original upstream failure"),
                        "upstream_timeout",
                    )
                }),
            )
        ...
        assert_eq!(original.0, 504);
        assert_eq!(original.1["message"], "original upstream failure");
        let repeated = send(address, "/v1/releases", body).await;
        assert_eq!(repeated.0, 504);
        assert_eq!(repeated.1["code"], "request_previous_failed");
        assert_eq!(repeated.1["request"]["error_code"], "upstream_timeout");
        server.abort();
        let _ = server.await;
        std::fs::remove_dir_all(root).unwrap();
    }
    ...
    async fn disconnected_client_owner_finishes_and_receipt_retains_original_context() {
        ...
        std::fs::write(
            &paths.profiles_file,
            br#"{"active_profile":"original-profile"}"#,
        )
        .unwrap();
        let state = Receipts::new(root.clone());
            ...
            .route(
                ...
                post(move || {
                    ...
                    async move {
                        ...
                        Json(json!({"current_release":"original-slot"}))
                    }
                }),
            )
            .layer(middleware::from_fn_with_state(state.clone(), enforce));
        ...
        let body = json!({"request_id":id,"action":"rollback","note":"SECRET-SENTINEL"});
        ...
        std::fs::write(
            &paths.profiles_file,
            br#"{"active_profile":"later-profile"}"#,
        )
        .unwrap();
        ...
        assert_eq!(
            repeated.1["request"]["context"]["profile"],
            "original-profile"
        );
        assert_eq!(
            repeated.1["request"]["context"]["version"],
            env!("CARGO_PKG_VERSION")
        );
        let summary = diagnostic_summary(&paths).to_string();
        assert!(!summary.contains("SECRET-SENTINEL"));
        assert!(!summary.contains("fingerprint"));
        assert!(!std::fs::read_to_string(root.join("request-receipts.json"))
            .unwrap()
            .contains("SECRET-SENTINEL"));
        // Records from before correlation metadata remain readable.
        let mut legacy: Value =
            serde_json::from_slice(&std::fs::read(root.join("request-receipts.json")).unwrap())
                .unwrap();
        legacy["requests"][0]
            .as_object_mut()
            .unwrap()
    ...
    fn launch_context_is_safe_and_old_session_has_unknown_metadata() {
        ...
        let paths = nexus_core::NexusPaths::from_root(root.clone());
        std::fs::create_dir_all(&paths.run_dir).unwrap();
        let mut session = nexus_core::HarnessLogSession::new(
            "run-fixture".into(),
            1,
            0,
            0,
        ...
        session.context = Some(nexus_core::OperationContext {
            build_id: Some("build-fixture".into()),
            version: Some("0.1.2".into()),
            profile: Some("effective-profile".into()),
            config_revision: Some("sha256:fixture".into()),
            run_id: Some(session.run_id.clone()),
        });
        nexus_core::HarnessLogSessionStore::new(paths.clone())
            .write(&session)
            .unwrap();
        ...
        assert!(nexus_core::HarnessLogSessionStore::new(paths)
            .read()
            .unwrap()
            .unwrap()
            .context
