...
- `cargo test --offline -p nexus-runtime-supply`: 15 passed, 0 failed. Coverage includes confirmation/stale/tampered plans, strict redirect identity, real Node signed-manifest fixture, real pnpm metadata signature/SRI, Corepack content change, ZIP traversal/case collision, tar regular extraction/link rejection, corruption, cancellation cleanup/no publication, concurrent cache reuse, stale lock-marker recovery, deterministic LTS selection, typed system plans, and system result classification.
