...
## P0 foundation, runtime supply, and snapshot-agent integration (2026-09-05)

The isolated P0 base combines the reviewed runtime-supply head
`ab8939e5dd121305c8f19af7c88637c984b20e7b` with the reviewed snapshot-agent
head `4cd3e556687c2d1f43008455cb5d44da54377538`, both based on
`f56c5e4b618ec9ee27c3baf2dc0e9dd4bac9da13`. Their implementation blobs are
preserved in the combined tree; source branches are retained. This is not
completion of P0.
...
Agent journal wiring, healthy capture hooks, and actual dependency materialization
are implemented in the integrated snapshot-agent head. Plugin and native Profile
adapters, and the recovery UI, remain pending. Manual capture can leave an
unidentified staging directory after a crash; inventory fails closed and requires
explicit cleanup rather than selecting or deleting an unproven candidate. Only
synthetic data and fault injection were used, including real Windows junction
fixtures. No physical power-cut guarantee was tested.
...
Runtime supply is integrated as a library, while cold clone/install/build/Node
launch, source confirmation, and native GUI acceptance remain P0 work. System
installer behavior is covered by a real implementation with an injected test
runner; this workstation's actual installation acceptance is restricted to
portable mode.
