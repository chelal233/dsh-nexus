...
在工作台启动或停止 Harness，并查看当前实例状态。浏览器与独立窗口是可选择的入口，独立窗口专用原生接口在浏览器中未必可用。DSH 终端使用所选版本和 profile；不要把全局安装的 `dsh` 命令误认为当前 Nexus 管理的版本。
...
**Nexus 更新**：开启自动更新时，每次应用启动检查一次，之后每两小时检查；关闭自动更新后仍可手动检测。发现更新后后台下载，按钮显示进度；下载校验完成后“更新”用于应用并重启。正在运行的 Harness 会被中止，应先结束重要任务。

**Harness 更新**：在更新页准备上游版本，准备与切换分开。切换前停止 Harness，按当前保护要求确认。不同版本可能使用不同数据格式，Nexus 不自动承诺数据降级兼容。

便携包也可以直接下载并完整解压新版。v0.1.7 已知有旧内置运行时路径残留问题；当前源码修复尚未包含在该发布包中，处理方式见[故障指南](troubleshooting.md)。
