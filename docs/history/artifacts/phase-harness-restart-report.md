...
Worktree: `<WORKSPACE>/dsh-nexus-harness-restart`
...
report. Harness/Desktop source, `<HARNESS_DATA>`, root Cargo manifests, and lock
...
| Rust formatting | PASS (exit 0) | `<USER_HOME>\.cargo\bin\cargo.exe fmt --all -- --check` |
| Rust workspace check | PASS (exit 0) | `<USER_HOME>\.cargo\bin\cargo.exe check --workspace --locked` |
| Rust workspace tests | PASS (exit 0) | `<USER_HOME>\.cargo\bin\cargo.exe test --workspace --locked`; 21 Agent, 15 Core, 11 Launcher, and 9 Protocol tests passed |
...
No deployment, merge, Harness/Desktop modification, `<HARNESS_DATA>` access, or
