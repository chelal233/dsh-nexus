...
Local inspection found that dshmarket 1.39.0 recognizes the `desktopProfiles` service from a different Desktop implementation. Official Harness 0.1.6-alpha.2 provides neither that service nor a `--profile` argument in its Host process, so the market falls back to `web` for installed listings and package operations. Its installed badge therefore does not prove that Desktop loads the plugin. Official Desktop still reads `profiles/desktop/package.json`; inspect that profile in Nexus. Upstream fixed profile selection and Desktop package operations in [dshmarket 1.52.0](https://github.com/dsh-market/dsh-market/releases/tag/v1.52.0), now pinned for first installation by Nexus. Existing installations are not automatically upgraded; update through an entry point explicitly targeting desktop, not the old market reading the wrong profile. Nexus does not rewrite user plugins or emulate another host contract.

## Multiple versions and write ownership

Nexus adapts to the selected Harness artifact's capabilities. A third-party plugin's version range is not the support range for Harness itself. Existing versions retain their original paths; widening a version allowlist does not verify a changed interface.

- Nexus installation, verification and repair own official program/runtime files. Package management owns the selected Profile. Before official Desktop invokes pnpm, Nexus unlinks package projections into official source, including private fallback links, without recursively deleting their targets. Configuration and external plugin links remain. This is not an OS sandbox for arbitrary plugins.
- Offline archives are verified distribution material; extracted runtimes may be shared. Official source-mode startup still prepares a runtime directory. This change does not replace that architecture or treat caches as user data.
- Legacy settings use `settings.yaml`; the 0.1.7 Profile settings service uses `profiles/<profile>/cordis.patch.yml`. Open Settings checks the selected artifact's compiled or source settings interface and reports unknown layouts explicitly, not a global `.imported` marker. Nexus neither performs the upstream first import nor reverse-converts migrated settings for an older release.
- The existing snapshot format covers both legacy settings and Profile patches. Restoring migrated settings matches sequence entries by unique id to preserve current secrets, refuses missing or ambiguous identities, does not restore historical secrets, and leaves `settings.yaml.imported` and session logs untouched. Retain a pre-upgrade snapshot together with the old runtime for rollback.
- Notifications subscribe to `session/event`, without synchronous history readers, V3/V4 file parsing, or `agent/session-start`. Harness owns session-format migration.

The relevant 0.1.7-rc.1 source contracts and local Nexus regressions were checked. Full new-version Desktop startup, real historical session migration and all third-party plugins have not been accepted. This adaptation does not automatically update the runtime lock or the user's active Harness.