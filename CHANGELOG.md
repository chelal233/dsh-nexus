...
- 正常启动先运行 Harness，只有明确的启动失败才进入诊断；可确认的缺失依赖最多自动修复并重试一次。启动检查超时保留诊断，不擅自停止仍在运行的进程。
- 官方 Desktop 启动前绑定当前版本的官方模块，并备份遮蔽它们的旧官方包，修复旧 settings 提供方与新版本接口冲突；保留第三方插件和配置。
- 验收发现内置 Git 的对应源码及嵌套通知尚未齐备，因此 CI 可继续构建验证，但安装包上传与正式发行暂不放行。
- Harness 未启动时也能管理插件。所选版本提供官方管理器时，清单、启用、禁用、检查、安装和移除复用其实现与保护规则。新安装插件保持禁用，需明确启用；禁用保留依赖。旧版本保留兼容入口。
- Node、pnpm 与完整便携 Git 优先使用显式配置，否则使用内置工具；配置档修复和终端操作无需先启动 Harness 才能找到运行时。完整离线导出保留这些工具。
- 快照与检查点标题使用本机时间，详情改为抽屉，默认可读配置，并提供“源码”视图。
- 同批此前修复包括安装完成后切换版本、按能力判断的冷启动加速、端口 0 随机分配、设置传递和可操作的启动诊断。
- 验证边界：Windows 整包验证与 macOS/Linux 真机验收分开。包脚本批准仍由 Harness 负责。Git 对应源码与嵌套许可的公开分发核查保留在第三方说明中；本条目不表示已发布 GitHub 版本。
